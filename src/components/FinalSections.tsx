...
import { Users, Layout, Activity, Code, Search, Radio, Smartphone, Microscope, ArrowRight } from 'lucide-react'
    ...
    <section id="company" className="bg-black py-32 border-t border-white/10">
      <div className="section-container">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
          <FadeIn>
            <div>
              <h2 className="text-5xl md:text-6xl font-bold mb-10 tracking-tight leading-tight">How We <span className="font-serif italic font-normal text-white/80">Operate</span></h2>
              <p className="text-brand-gray-400 text-xl font-light leading-relaxed mb-8">
                  ...
                  <div className="flex items-center gap-10 p-8 bg-black group hover:bg-brand-gray-900 transition-all">
                    ...
                    <div className="flex-1 flex items-center justify-between">
                      ...
                      <div className="text-brand-gray-600 group-hover:text-brand-orange transition-colors">
                         {React.cloneElement(step.icon as React.ReactElement, { className: 'w-6 h-6 stroke-[1.5px]' })}
                      </div>
                    </div>
                  </div>
                </StaggerItem>
      ...
      <div className="section-container">
        <FadeIn direction="none">
          <div className="text-center max-w-3xl mx-auto mb-24">
             <h2 className="text-5xl md:text-6xl font-bold mb-8 tracking-tight">Data Science & <span className="font-serif italic font-normal text-white/80">Consulting</span></h2>
             <p className="text-brand-gray-400 text-lg font-light leading-relaxed">Tailored, domain-specialized AI: from custom pre-training with your data to scaled deployment—with expert guidance throughout.</p>
          </div>
        </FadeIn>
        <StaggerContainer>
              ...
              <StaggerItem key={i}>
                <div className="group p-10 border-r border-b border-white/10 hover:bg-brand-gray-900 transition-all h-full">
                  <div className="text-brand-gray-600 group-hover:text-brand-orange mb-10 transition-colors">
                    {React.cloneElement(s.icon as React.ReactElement, { className: 'w-10 h-10 stroke-[1px]' })}
                  </div>
                  <h3 className="text-xl font-bold mb-4 tracking-tight group-hover:text-white transition-colors">{s.title}</h3>
                  <p className="text-sm text-brand-gray-500 leading-relaxed font-light">{s.desc}</p>
                </div>
              </StaggerItem>
            ))}
          </div>
    ...
    <section className="bg-black py-32 border-t border-white/10 overflow-hidden">
      <div className="section-container">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24">
          <FadeIn>
            <div>
              <h2 className="text-5xl md:text-6xl font-bold mb-10 tracking-tight leading-tight">Why Organizations <br />Choose <span className="font-serif italic font-normal text-white/80">ANTERA</span></h2>
              ...
              {reasons.map((reason, i) => (
                <StaggerItem key={i}>
                  <div className="group flex items-start gap-6 border-b border-white/5 pb-8 h-full">
                    <div className="mt-1 font-mono text-[10px] text-brand-orange">0{i + 1}</div>
                    <span className="font-bold text-xl tracking-tight text-white/90 group-hover:text-brand-orange transition-colors">{reason}</span>
                  </div>
                </StaggerItem>
              ))}
            </div>
