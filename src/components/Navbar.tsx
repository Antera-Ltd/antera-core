...
export const Navbar = () => {
  ...
  return (
    <>
      {/* Top Header */}
      <nav 
        className="fixed top-0 left-0 right-0 z-50 bg-white border-b-4 border-black text-xs font-mono font-bold antialiased uppercase tracking-wider"
      ...
      >
        <div className="mx-auto flex items-stretch justify-between h-16">
          ...
          <div className="hidden lg:flex items-stretch">
            {/* Language Switcher */}
            <div className="relative flex items-stretch border-l-4 border-black">
              <button
                id="lang-selector"
              ...
              <AnimatePresence>
                {showLangs && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 4 }}
                    className="absolute right-0 top-[60px] bg-white border-4 border-black shadow-[4px_4px_0px_0px_#000000] w-24 flex flex-col divide-y-2 divide-black"
                  >
                    {languages.map((lang) => (
                      <button
                        key={lang.code}
            ...
            <button className="px-6 flex items-center gap-2 border-l-4 border-black text-black hover:bg-neutral-50 transition-colors">
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="lg:hidden px-6 flex items-center justify-center text-black border-l-4 border-black hover:bg-neutral-50"
        ...
        <AnimatePresence>
          {activeMenu === 'Blog' && (
            <motion.div
              ...
              className="hidden lg:block absolute left-0 right-0 top-full bg-white border-b-4 border-black z-40"
            >
              <div className="flex max-w-4xl border-r-4 border-black">
                
                {/* Left Column: Latest Posts */}
                <div className="w-7/12 border-r-4 border-black flex flex-col bg-white">
                  <div className="px-6 py-3 text-[10px] uppercase font-bold tracking-wider text-neutral-400 bg-neutral-50 border-b-2 border-black">
                    Latest Briefings
                  </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-16 bg-white z-40 lg:hidden flex flex-col divide-y-4 divide-black border-t-4 border-black overflow-y-auto font-mono font-bold text-xs uppercase tracking-wider"
          >
            <div className="flex flex-col divide-y-2 divide-black text-black">
              ...
              <div className="px-6 py-4 bg-neutral-50 flex items-center gap-4 border-b-2 border-black">
                <Globe className="w-4 h-4 stroke-[2.5px] text-neutral-400" />
                {languages.map(lang => (
                  <button
                    key={lang.code}
                    onClick={() => setLanguage(lang.code)}
                    className={`px-2 py-0.5 border-2 border-black ${language === lang.code ? 'bg-[#FA520F] text-white shadow-[2px_2px_0px_0px_#000000]' : 'bg-white text-black'}`}
                  >
                    {lang.name}
                  </button>
                ))}
            ...
            <div className="mt-auto bg-neutral-50 flex flex-col divide-y-2 divide-black border-t-4 border-black">
